...
#----------------------------------
# Note that because this is pulling 1,500+ images from the MIDN server, this can be slow. If you have all
...
years_MIDN1 <- c(2023, 2026)
years_MIDN2 <- c(2023, 2026)
years_NCBN <- c(2023, 2026)
years_COLO <- c(2023, 2026)
years_ASIS <- c(2023, 2026)
years_SAHI <- 2023

#----- Compile data for the popups -----
plots1 <- rbind(joinLocEvent(park = MIDN1, from = years_MIDN1[1], to = years_MIDN1[2], output = 'verbose', locType = 'all'), #RICH new plots
...
path <- c('./Monitoring/ForestVegetation/')
...
full_names <- map2_dfr(park_year$year, park_year$park, function(y, p){
  data.frame(full_name = list.files(paste0(path, y, "/03_Data/Photos/PhotoPoints/", p),
                                    pattern = 'JPG$|jpeg$|JPEG$|jpg$', full.names = T, 
                                    ignore.case = T))
})
...
name_df1 <- data.frame(full_name = full_names$full_name, 
                       photo_name = 
                         ifelse(grepl("JPG", full_names$full_name, ignore.case = T), 
                                substr(full_names$full_name, nchar(full_names$full_name) - 23, nchar(full_names$full_name)),
                                substr(full_names$full_name, nchar(full_names$full_name) - 24, nchar(full_names$full_name)))
                       )
